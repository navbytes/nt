<!--
  ...
  nt and is injected into context automatically by the nt-memory extension (compiled
  ...
  Pi concatenates AGENTS.md (or CLAUDE.md) from ~/.pi/agent/, parent directories,
  and the current directory. Place this at ~/.pi/agent/AGENTS.md (global) or
...
it on demand. nt's operations are available as `nt_*` tools (the nt-memory
extension bridges nt's MCP server in as native Pi tools).
...
- Use the **`nt` skill** (`/skill:nt`) for the full workflow and the folder/tag
  conventions (`rules/`+`rule`, `memory/`+`memory-core`, everything else =
  on-demand KB).

## Session review
- `/learn` reviews the session and saves approved learnings to nt; `/recall`
  loads a task-priming or resume brief on demand. If a session did real work but
  captured nothing, the extension nudges you to run `/learn` before it ends.
