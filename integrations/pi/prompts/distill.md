...
most. Optional focus: "$@" (if non-empty, only review pairs touching it).
...
distill`, `nt show`, `nt edit --append`, `nt supersede <old> --by <new>`, `nt
tag <id> +distinct`); if that's missing too, tell the user to run `nt pi
install` and stop.
