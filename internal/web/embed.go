...
// the binary so `nt web` (and the desktop app) is a single static binary with no
// runtime dependency. The bundle is a build artifact: it must be built (`make
// web-build`) and committed so `go build`/`go install` — which never run npm —
// still embed a current UI. CI verifies the committed bundle matches a fresh
// build. `all:` includes files Vite may emit with leading underscores/dots.
//
