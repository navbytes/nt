...
// the binary so a built `nt`/desktop app is a single static binary with no
// runtime dependency. The bundle is a build artifact generated by `make
// web-build` (run automatically by CI and the release pipeline); it is NOT
// committed — only a non-dot `placeholder` file is (dot-files are dropped from
// Go module zips, which would break `go install`), so this embed still compiles
// on a checkout that hasn't run the build yet (e.g. a Node-free `go install`). When
// the bundle is absent, spaRoutes registers no UI routes — its error is
// intentionally ignored by the caller (server.go) — so `nt web` starts but
// serves no frontend until it's built. `all:` includes files Vite may emit with
