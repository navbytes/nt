...
- Notes/rules/memory/lessons → `nt_note` (title, `description`, `body`;
  ...
  tag+folder). Set `source:"claude"`, and pass **`if_exists:"return"`** — on
  ...
  a conclusion, record why with `nt_decide`. Give volatile facts a
  `half_life` (e.g. `"90d"`) so they fade instead of going stale. Apply the
  user's edits.
