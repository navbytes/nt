...
- **MCP server** — `nt mcp` exposes **15 typed tools** (`nt_index`, `nt_recall`, `nt_add`, `nt_note`, `nt_get`, `nt_status`, `nt_search`, `nt_update`, …) with strict unknown-parameter rejection, over stdio. Register it with one command:
  ```bash
