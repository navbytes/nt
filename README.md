...
- **MCP server** — `nt mcp` exposes **22 typed tools** (`nt_index`, `nt_recall`, `nt_add`, `nt_note`, `nt_note_edit`, `nt_get`, `nt_status`, `nt_search`, `nt_update`, `nt_doctor`, `nt_distill`, `nt_mindmap`, …) with strict unknown-parameter rejection, over stdio. Register it with one command:
  ```bash
